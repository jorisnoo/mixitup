const renderElement = (html: string): HTMLElement => {
...
export function getEmptyContainer(): HTMLElement {
    return renderElement('<div class="container" data-ref="container"></div>');
}

export function getFilterControls(): HTMLElement {
...
export function getMultimixControls(): HTMLElement {
    return renderElement('<div class="mixitup-controls">' +
...
export function getTotalWhitespace(html: string): number {
    const re = /[>? ]( )[<? ]/g;
...
export const ITEM_TEMPLATE = '<div id="${id}" class="${classList}" data-ref="mix" data-category="${categoryList}" data-published="${published}" data-views="${views}"></div>';

export const ITEM_TEMPLATE_ALT = '<div id="${id}"></div>';
